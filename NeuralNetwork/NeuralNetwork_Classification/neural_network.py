...
class NeuralNetwork():
    ...
    def fprop(self, batch_input):
        ...
        for i in range(1, self.num_layers + 1):
            ...
            #if self.activations_func[i]=="softmax":# in this case output layer which activation is just output
    ...
    def bprop(self, batch_target,epoch):
        beta = 0.9
...
#             print("layer %s"%l)
    ...
    def train(self, train_x, train_y):
        ...
        for i in range(0, self.num_iterations):
            for idx in range(0, num_samples, self.mini_batch_size):
                ...
                self.fprop(minibatch_input.T)
                loss = self.calculate_loss(minibatch_target.T)
                self.bprop(minibatch_target.T,i)           
    ...
    def test(self,x,y):
        ...
        for n in range(len(x)):
            self.fprop(np.reshape(x[n], (784,1)),True)
